  ...
  const events = [
    {
      ...
      description:
        "Learn the latest digital marketing strategies and tools in this hands-on workshop.",
      type: "Workshop",
      icon: "📱",
    ...
    },
    {
      ...
      location: "Tech Bridge Pakistan Campus",
      description:
        "Get expert advice on career planning and job search strategies in the tech industry.",
      type: "Seminar",
      icon: "🎯",
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-blue-50 to-indigo-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5">
        ...
        <div
          className="absolute bottom-20 right-10 w-64 h-64 bg-purple-500 rounded-full mix-blend-multiply animate-float"
          style={{ animationDelay: "2s" }}
        ></div>
        <div
          className="absolute top-1/2 left-1/2 w-64 h-64 bg-pink-500 rounded-full mix-blend-multiply animate-float"
      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          {events.map((event, index) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4 sm:mb-6">
                <div
                  className={`w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 bg-gradient-to-r ${event.gradient} rounded-xl sm:rounded-2xl flex items-center justify-center text-lg sm:text-xl lg:text-2xl shadow-lg group-hover:scale-110 transition-transform-smooth`}
                >
                  {event.icon}
                </div>
                <div className="text-right">
                  <div
                    className={`inline-block px-2 sm:px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r ${event.gradient} text-white`}
                  >
                    {event.type}
                  </div>
                </div>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                {/* Register Button */}
                <Button
                  className={`w-full bg-gradient-to-r ${event.gradient} hover:shadow-lg text-white py-2 sm:py-3 px-4 sm:px-6 rounded-lg sm:rounded-xl text-sm sm:text-base font-semibold transition-all-smooth group-hover:scale-105`}
                >
                  Register Now
        ...
        >
          <Link href="/events">
            <Button
              size="lg"
              className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white px-6 sm:px-12 py-3 sm:py-4 rounded-lg sm:rounded-full text-sm sm:text-lg font-semibold shadow-xl hover-lift transition-all-smooth"
            >
              View All Events
              <span className="ml-2">📅</span>
            </Button>
